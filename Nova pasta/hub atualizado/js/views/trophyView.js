...
export function renderTrophyView(container) {
   ...
   // --- EVENTS ---
    ...
    // CORREÇÃO 2: Lógica de Salvamento "À Prova de Falhas" para o Casarão
    safeAddEvent('btn-modal-save', 'onclick', () => {
        try {
            ...
            // Lógica Condicional (Casarão vs Outros)
            if (isLodge) {
                // No Casarão, ignoramos o dropdown de animais
                finalAnimal = 'CASARÃO';
                
                // Se o título for vazio, definimos um padrão (já que a UI diz que é Opcional)
                if (!finalTitle) {
                    finalTitle = 'Registro do Casarão';
                }
            } else {
                // Nas outras abas, o animal é obrigatório
                if (!animalEl || !animalEl.value) {
                    return showToast('SELECIONE UM ANIMAL!', 'warning');
                }
                finalAnimal = animalEl.value;
                finalScore = scoreEl ? scoreEl.value : '';
                finalGender = genderEl ? genderEl.value : '';
                finalFur = furEl ? furEl.value : '';
            }
            
            // Tratamento da Data
            const rawDate = dateEl ? dateEl.value : null;
            const displayDate = rawDate ? new Date(rawDate + 'T12:00:00').toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR');
            ...
            // Captura de Stats (se visível)
            const statsPanel = document.getElementById('um-stats-panel');
            if (!isLodge && statsPanel && statsPanel.style.display !== 'none') {
                newTrophy.sessionStats = {
                    kills: document.getElementById('um-stat-kills')?.value || 0,
                    diamonds: document.getElementById('um-stat-diamonds')?.value || 0,
                    rares: document.getElementById('um-stat-rares')?.value || 0,
                    trolls: document.getElementById('um-stat-trolls')?.value || 0
                };
            }

            // Persistência
            console.log("Salvando Troféu:", newTrophy); // Debug
    ...
    safeAddEvent('btn-modal-cancel', 'onclick', () => document.getElementById('upload-modal').style.display = 'none');
