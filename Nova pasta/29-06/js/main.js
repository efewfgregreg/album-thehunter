...
export function renderMainView(tabKey, addToHistory = true) {
    ...
    // --- CORREÇÃO: Lógica de Cabeçalho com Suporte a Horário Robusto ---
    // Verifica se há Grind ativo no objeto global window ou no estado salvo
    const horarioAtual = (typeof window !== 'undefined' && window.activeGrindData?.horario) 
        ? window.activeGrindData.horario 
        : (savedData.activeSession?.horario || null);

    const header = createPageHeader(title, () => renderNavigationHub(), horarioAtual);
    ...
    } else if (tabKey === TABS.ROTINA) {
        import('./views/routineHubView.js').then(module => {
            module.renderRoutineHubView(content, (target) => renderMainView(target));
        });
    }
