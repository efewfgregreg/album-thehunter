...
function openGrindDetailModal(sessionId, type, killCount) {
    ...
    modalContent.style.cssText = `max-width: 900px; width: 95%; background: #181818; border-radius: 12px; display: flex; flex-direction: column; max-height: 90vh; overflow: hidden; position: relative;`;
    
    modalContent.innerHTML = `
        <div class="modal-header-simple" style="padding: 20px; flex-shrink: 0;">
            <h3 style="color: var(--primary-color); margin: 0; font-family: var(--font-headings);">${title}</h3>
            <p style="color: #888; margin: 5px 0 0 0;">${animalName}</p>
        </div>
        <div class="grind-select-grid" style="flex: 1; overflow-y: auto; padding: 0 20px 20px 20px; display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 15px;"></div>
        <div class="modal-buttons" style="padding: 20px; border-top: 1px solid #333; display: flex; justify-content: center; background: #1e1e1e; flex-shrink: 0;">
            <button id="btn-cancel-select" class="back-button" style="width: 100%; max-width: 300px; padding: 12px;">CANCELAR</button>
        </div>
    `;
    ...
    potentialFurs.forEach((fur) => {
        // 1. Precisamos reconstruir o nome do arquivo igual ao que está na sua pasta
        // O slugify converte "Macho Leucismo" para "macho_leucismo"
        ...
        // 2. Monta o nome conforme vemos na sua pasta de arquivos
        let fileName = `${animalSlugFixed}_${furSlug}${genderPart}`;

        // 3. Caso especial para Great Ones (que você mostrou que tem "great_" no início)
        if (type === 'great_ones') {
            fileName = `great_${animalSlugFixed}_${furSlug}`;
        }
        ...
        card.style.cssText = `display: flex; flex-direction: column; align-items: center; padding: 10px; background: #222; border: 1px solid #333; border-radius: 8px; cursor: pointer; transition: 0.2s; min-height: 140px;`;

        const imgContainer = document.createElement('div');
        imgContainer.style.cssText = "width:100%; height:100px; display:flex; align-items:center; justify-content:center; overflow:hidden; margin-bottom:8px; background: #111; border-radius: 4px;";
        
        const img = new Image();
        img.src = imgUrl;
        
        // Log de depuração para verificar o caminho que está falhando
        img.onerror = () => { 
            console.warn("Imagem não encontrada, tentando fallback:", imgUrl);
            img.src = `animais/${animalSlugFixed}.png`; 
        };
        ...
        nameDiv.style.cssText = "font-size: 0.75rem; color: #eee; text-align: center; line-height: 1.2; word-break: break-word; font-family: var(--font-headings); padding: 5px 0;";
        ...
        card.onclick = () => {
             // ... sua lógica de clique original ...
             const displayName = fur.displayName;
             // ... (resto da lógica)
             saveData(savedData);
             closeModal('form-modal');
             renderGrindCounterView(sessionId);
             showToast(`${displayName} salvo com sucesso!`);
        };
