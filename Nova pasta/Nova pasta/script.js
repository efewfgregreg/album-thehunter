...
"new_england_mountains": {
    ...
    "lince_pardo_do_mexico": { "maxScore": "27.68", "maxWeightEstimate": "35-45 KG", "drinkZonesPotential": "03:00 - 06:00", "animalClass": "2", "maxLevel": "9 (Lendário)" },
    "raposa_cinzenta": { "maxScore": "6.43", "maxWeightEstimate": "5-6 KG", "drinkZonesPotential": "17:00 - 20:00", "animalClass": "2", "maxLevel": "9 (Lendário)" },
    "veado_de_cauda_branca": { "maxScore": "255.09", "maxWeightEstimate": "75-100 KG", "drinkZonesPotential": "08:00 - 12:00", "animalClass": "4", "maxLevel": "3 (Muito Fácil)" },
    "urso_negro": { "maxScore": "22.8", "maxWeightEstimate": "227-290 KG", "drinkZonesPotential": "04:00 - 08:00", "animalClass": "7", "maxLevel": "9 (Lendário)" },
    "coiote": { "maxScore": "56.87", "maxWeightEstimate": "24-27 KG", "drinkZonesPotential": "00:00 - 04:00", "animalClass": "2", "maxLevel": "9 (Lendário)" },
...
const categorias = {
    ...
    configuracoes: { title: 'Configurações', icon: '<i class="fas fa-cog"></i>', isHtml: true }
};
...
// Função para renderizar o Skeleton Loader
    ...
    appContainer.innerHTML = `
        <div class="skeleton-loader-container">
            ...
            <div class="skeleton-cards-container">
                ${Array(8).fill(0).map(() => `
                    <div class="skeleton skeleton-nav-card"></div>
                `).join('')}
            </div>
        </div>
    `;
}
// Em script.js, dentro da função renderNavigationHub()
...
function renderNavigationHub() {
    ...
    // Código do Dashboard (que já estava correto)
    const overallProgress = calcularOverallProgress();
    const dashboard = document.createElement('div');
    dashboard.className = 'hub-dashboard';
    dashboard.innerHTML = `
        <div class="dashboard-stat">
            <img src="icones/pata_icon.png" class="custom-icon">
            <div class="stat-info">
                <span class="stat-value">${overallProgress.collectedRares} / ${overallProgress.totalRares}</span>
                <span class="stat-label">Peles Raras</span>
            </div>
        </div>
        <div class="dashboard-stat">
            <img src="icones/diamante_icon.png" class="custom-icon">
            <div class="stat-info">
                <span class="stat-value">${overallProgress.collectedDiamonds} / ${overallProgress.totalDiamonds}</span>
                <span class="stat-label">Diamantes</span>
            </div>
        </div>
        <div class="dashboard-stat">
            <img src="icones/greatone_icon.png" class="custom-icon">
            <div class="stat-info">
                <span class="stat-value">${overallProgress.collectedGreatOnes} / ${overallProgress.totalGreatOnes}</span>
                <span class="stat-label">Great Ones</span>
            </div>
        </div>
        <div class="dashboard-stat">
            <img src="icones/coroa_icon.png" class="custom-icon">
            <div class="stat-info">
                <span class="stat-value">${overallProgress.collectedSuperRares} / ${overallProgress.totalSuperRares}</span>
                <span class="stat-label">Super Raros</span>
            </div>
        </div>
    `;
    hub.appendChild(dashboard);
    ...
    appContainer.appendChild(hub); // <-- A linha mais importante que faltava!
...
function renderSimpleDetailView(name, tabKey) {
    ...
    renderContent('all'); // Renderiza o conteúdo inicial
}
...
function renderAnimalDossier(animalName, originReserveKey) {
    ...
    const reRenderActiveTab = (filter) => {
        ...
        if (activeTab) {
            ...
            if (renderFunc) {
                // Para a aba de hotspot, apenas chamamos a função sem passar o container
                if (tabKey === 'hotspot') {
                    renderFunc();
                } else {
...
function renderHotspotDetailModal(reserveKey, animalSlug) {
    ...
    // Mantém a funcionalidade de zoom e arrastar a imagem
    if (window.panzoom) {
 ...
 * E INCLUI um sumário de progresso diretamente no card.
 */
function updateCardAppearance(card, slug, tabKey) {
    if (!card) return;
    card.classList.remove('completed', 'inprogress', 'incomplete');

    // Remove sumário antigo para evitar duplicatas ao re-renderizar
    const oldSummary = card.querySelector('.card-progress-summary');
    if (oldSummary) oldSummary.remove();

    let status = 'incomplete';
    let summaryHTML = ''; // Variável para guardar o HTML do sumário
    ...
    switch (tabKey) {
        case 'greats':
            ...
            summaryHTML = `<img src="icones/greatone_icon.png" class="custom-icon"> <span>${collectedFurs} / ${totalGreatFurs}</span>`;

            if (animalData.completo) {
                status = 'completed';
            } else if (collectedFurs > 0) {
                status = 'inprogress';
            }
            break;
        ...
        case 'diamantes':
            ...
            const highestScore = collectedDiamondTrophies.reduce((max, t) => Math.max(max, t.score), 0);
            
            if (highestScore > 0) {
                summaryHTML = `<i class="fas fa-trophy"></i> <span>${highestScore.toFixed(2)}</span>`;
            }
        ...
        case 'super_raros':
            ...
            if (speciesRareFursForSuper) {
                ...
                summaryHTML = `<img src="icones/coroa_icon.png" class="custom-icon"> <span>${collectedCount} / ${totalCount}</span>`;
        ...
        case 'pelagens':
            ...
            if (speciesRareData) {
                ...
                summaryHTML = `<img src="icones/pata_icon.png" class="custom-icon"> <span>${collectedCount} / ${totalCount}</span>`;
    ...
    // Adiciona o sumário ao card se houver algum conteúdo
    if (summaryHTML) {
        const summaryDiv = document.createElement('div');
        summaryDiv.className = `card-progress-summary ${tabKey}`; // Adiciona a key da aba como classe para estilo específico
...
function renderRareFursDetailView(container, name, slug, originReserveKey = null, filter = 'all') {
    ...
    genderedFurs.sort((a, b) => a.displayName.localeCompare(b.displayName)).forEach(furInfo => {
        ...
        furCard.addEventListener('click', () => {
            ...
            if (originReserveKey) {
                 reRenderActiveDossierTab(originReserveKey, name, slug);
            } else {
        ...
        scoreContainer.addEventListener('click', e => {
            ...
            const saveScore = () => {
                ...
                if (originReserveKey) {
                    ...
                    renderSimpleDetailView(name, 'diamantes');
                }
            };
...
// Helper para re-renderizar a aba ativa do dossiê
function reRenderActiveDossierTab(reserveKey, animalName, slug) {
    ...
    if (activeTab) {
        ...
        // Recriamos o objeto 'tabs' aqui para ter acesso às funções de renderização
        const tabs = {
...
function renderSettingsView(container) {
    ...
    resetButton.onclick = async () => {
        if (await showCustomAlert('Tem certeza que deseja apagar TODO o seu progresso? Esta ação não pode ser desfeita.', 'Resetar Progresso', true)) {
            ...
            saveData(savedData);
            // Adicionado para recarregar a página e refletir o reset
            location.reload(); 
        }
    };
            ...
            <div class="grind-header">
                ...
                <div class="grind-header-details">
                    ...
                    <div class="detail-item">
                        <button class="hotspot-button" onclick="renderHotspotDetailModal('${reserveKey}', '${animalSlug}')">
...
function renderGrindHubView(container) {
    ...
    if (savedData.grindSessions && savedData.grindSessions.length > 0) {
        ...
        const sortedReserveKeys = Object.keys(grindsByReserve).sort((a, b) => {
            const nameA = reservesData[a]?.name || '';
            const nameB = reservesData[b]?.name || '';
        ...
        emptyStateContainer.innerHTML = `
            <div class="empty-state-container">
                ...
                <p class="empty-state-message">
                    O Contador de Grind ajuda a rastrear seus abates, diamantes e raros enquanto você caça um troféu específico. Crie seu primeiro grind agora!
                </p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('scroll', () => {
        if (window.scrollY > 300) { // Mostra o botão depois de rolar 300 pixels
