...
function createAnimalCard(name, tabKey) {
    ...
    // A aparência é atualizada após a criação, se necessário, ou fica como 'incomplete'
...
function renderTrophyList(fur, slug, tabKey, onListChangeCallback) {
    ...
    if (trophies.length > 0) {
        trophies.forEach((trophy, index) => {
            ...
            removeBtn.onclick = (e) => {
                ...
                saveData(savedData);
                renderTrophyList(fur, slug, tabKey, onListChangeCallback); 
    ...
    addBtn.onclick = () => showAddTrophyForm(fur, slug, tabKey, onListChangeCallback);
    listWrapper.appendChild(addBtn);

    container.appendChild(listWrapper);
}

function showAddTrophyForm(fur, slug, tabKey, onListChangeCallback) {
    const container = document.getElementById('trophy-list-container');
    container.innerHTML = '';
...
function updateCardAppearance(card, slug, tabKey) {
    ...
    if (tabKey === 'greats') {
        if (animalData.completo) { // Usa o flag 'completo' que é setado pelo checkAndSet...
            isComplete = true;
        }
    }
    // Adicionar outras lógicas para outras abas
    if (isComplete) {
...
function updateProgressPanel() {
    ...
    // Lógicas de contagem para cada categoria...
    // Pelagens
    ...
    const progressPelagens = document.getElementById('progress-pelagens');
    if(progressPelagens) {
        progressPelagens.value = obtidasPelagens; progressPelagens.max = totalPelagens;
        document.getElementById('stats-pelagens').textContent = `Obtidas: ${obtidasPelagens} / ${totalPelagens}`;
        document.getElementById('missing-pelagens').textContent = `Faltam: ${totalPelagens - obtidasPelagens}`;
    }
    // Diamantes
    const totalDiamantes = categorias.diamantes.items.length;
    ...
    const progressDiamantes = document.getElementById('progress-diamantes');
    if(progressDiamantes){
        progressDiamantes.value = obtidosDiamantes; progressDiamantes.max = totalDiamantes;
        document.getElementById('stats-diamantes').textContent = `Obtidos: ${obtidosDiamantes} / ${totalDiamantes}`;
        document.getElementById('missing-diamantes').textContent = `Faltam: ${totalDiamantes - obtidosDiamantes}`;
    }
    // Great Ones
    ...
    if(progressGreats){
        progressGreats.value = obtidosGreats; progressGreats.max = totalGreats;
