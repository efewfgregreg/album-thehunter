...
// ... (TODAS AS CONSTANTES DE DADOS COMO 'rareFursData', 'greatsFursData', etc. CONTINUAM IGUAIS) ...
// Copie e cole todas as suas constantes de dados aqui (rareFursData, greatsFursData, items, diamondFursData)
...
let appContainer; // Alterado de mainContent para appContainer

// ... (O resto das suas funções checkAndSetGreatOneCompletion, createAnimalCard, render*DetailView, etc. continuam aqui) ...
// Copie e cole aqui todas as suas outras funções, pois elas não precisam de grandes alterações.
// A única alteração significativa será nas funções renderMainView e na inicialização.
...
// NOVA FUNÇÃO para renderizar o menu principal
function renderNavigationHub() {
    appContainer.innerHTML = ''; // Limpa o container principal
...
// FUNÇÃO MODIFICADA para renderizar as telas de conteúdo
function renderMainView(tabKey) {
    appContainer.innerHTML = ''; // Limpa o container principal
    const currentTab = categorias[tabKey];
    if (!currentTab) return;

    // Container para a visualização principal
    ...
    backButton.addEventListener('click', renderNavigationHub); // Volta para o Hub
...
// COLE TODAS AS OUTRAS FUNÇÕES (renderRareFursDetailView, renderDiamondsDetailView, etc.) AQUI SEM ALTERAÇÕES
...
// NOVO PONTO DE INÍCIO DO SCRIPT
document.addEventListener('DOMContentLoaded', () => {
    appContainer = document.getElementById('app-container');
    
    // Inicia a aplicação mostrando o Hub de Navegação
    renderNavigationHub();
