...
function openGrindDetailModal(sessionId, rarityType) {
    ...
    switch (rarityType) {
        ...
        case 'super_rares':
            ...
            if (speciesRareFurs && speciesDiamondFurs) {
                if (speciesRareFurs.macho && speciesDiamondFurs.macho && speciesDiamondFurs.macho.length > 0) {
                    speciesRareFurs.macho.forEach(rareFur => {
                        options.push(`Macho ${rareFur} Diamante`); // Mantido o sufixo "Diamante" aqui
                    });
                }
                if (speciesRareFurs.femea && speciesDiamondFurs.femea && speciesDiamondFurs.femea.length > 0) {
                    speciesRareFurs.femea.forEach(rareFur => {
                        options.push(`Fêmea ${rareFur} Diamante`); // Mantido o sufixo "Diamante" aqui
                    });
                }
            }
            break;
    ...
    modal.querySelector('#grind-detail-save').onclick = () => {
        ...
        // Note: For super_rares, we now expect the selectedValue to already contain " Diamante" if it's a super rare.
        // The display name "Macho Albino Diamante" becomes the key.
        const logDetails = {    
            variation: selectedValue, // The full name, e.g., "Macho Albino Diamante"
