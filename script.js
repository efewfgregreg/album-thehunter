...
const diamondFursData = {
    "alce": { macho: ["Bronzeado", "Pardo", "Pardo Claro", "Albino"], femea: ["Albino"] },
    ...
    "carneiro_selvagem": { macho: ["Preto", "Pardo", "Cinzento", "Bronze", "Malhado variação 1", "Malhado variação 2"], femea: ["Malhado variação 1", "Malhado variação 2"] },
    // CORREÇÃO: castor_norte_americano - APENAS FÊMEAS
    "castor_norte_americano": { macho: [], femea: ["Pardo Claro", "Pardo Escuro", "Marrom Avermelhado"] },
    ...
    // CORREÇÃO: codorniz_da_virgínia - APENAS FÊMEAS
    "codorniz_da_virgínia": { macho: [], femea: ["Pardo"] },
    // CORREÇÃO: coelho_da_flórida - MACHO E FÊMEA
    ...
    "crocodilo_de_água_salgada": { macho: ["Cinzento", "Oliva", "Pardo Escuro", "Leucismo"], femea: ["Leucismo"] },
    ...
    // CORREÇÃO: lebre_da_eurásia - MACHO E FÊMEA
    "lebre_da_eurásia": { macho: ["Branco", "pardo claro", "pardo escuro", "cinza claro", "cinza escuro"], femea: ["Branco", "pardo claro", "pardo escuro", "cinza claro", "cinza escuro"] },
    // CORREÇÃO: lebre_europeia - APENAS FÊMEAS
    "lebre_europeia": { macho: [], femea: ["cinza", "pardo", "pardo escuro", "pardo claro"] },
    // CORREÇÃO: lebre_nuca_dourada - APENAS FÊMEAS
    "lebre_nuca_dourada": { macho: [], femea: ["Castanho", "Pardo", "Cinzento"] },
    // CORREÇÃO: lebre_peluda - MACHO E FÊMEA
    ...
    // CORREÇÃO: oryx_do_cabo - MACHO E FÊMEA
    ...
    // CORREÇÃO: raposa_cinzenta - MACHO E FÊMEA
    ...
    // Assegurar que os últimos animais da sua lista original também estão aqui se aplicável
};
// **FIM DOS DADOS DE PELAGENS E DIAMANTES**
...
// **FIM DOS DADOS DE RESERVAS E MONTAGENS**
...
const categorias = {
    ...
    // A lista de Super Raros AGORA conterá TODOS os animais, o filtro de "pode ser diamante" é na tela de detalhes.
    super_raros: {
        title: 'Super Raros',
...
function renderSuperRareDetailView(container, name, slug) {
    ...
    // Lógica para incluir APENAS pelagens raras de gêneros que podem ser diamante
    if (speciesRareFurs?.macho && speciesDiamondFurs?.macho && speciesDiamondFurs.macho.length > 0) {
        speciesRareFurs.macho.forEach(rareFur => {
            genderedSuperRares.push({ displayName: `Macho ${rareFur}`, originalName: rareFur, gender: 'macho' });
        });
    }

    if (speciesRareFurs?.femea && speciesDiamondFurs?.femea && speciesDiamondFurs.femea.length > 0) {
        speciesRareFurs.femea.forEach(rareFur => {
            genderedSuperRares.push({ displayName: `Fêmea ${rareFur}`, originalName: rareFur, gender: 'femea' });
        });
    }

    if (genderedSuperRares.length === 0) {
        furGrid.innerHTML = '<p>Este animal não possui combinações de Pelagem Rara que podem gerar um Diamante para nenhum dos sexos.</p>';
        return;
    }

    genderedSuperRares.sort((a, b) => a.displayName.localeCompare(b.displayName)).forEach(furInfo => {
        const furCard = document.createElement('div');
        // A lógica de "completado" aqui será se a pelagem rara foi marcada
        ...
        scoreContainer.addEventListener('click', e => {
            ...
            const saveScore = () => {
                ...
                if (!isNaN(scoreValue) && scoreValue > 0) {
                    // Update existing or add new trophy
                    let existingTrophyIndex = savedData.diamantes[slug].findIndex(t => t.type === fullTrophyName);
                    if (existingTrophyIndex > -1) {
                        // Only update if new score is higher
                        if (scoreValue > savedData.diamantes[slug][existingTrophyIndex].score) {
                            savedData.diamantes[slug][existingTrophyIndex].score = scoreValue;
                        }
                    } else {
                        savedData.diamantes[slug].push({ id: Date.now(), type: fullTrophyName, score: scoreValue });
                    }
                } else if (isNaN(scoreValue) || scoreValue <= 0) {
                    // Remove trophy if score is invalid or 0
                    savedData.diamantes[slug] = savedData.diamantes[slug].filter(t => t.type !== fullTrophyName);
                }
...
function updateCardAppearance(card, slug, tabKey) {
    ...
    switch (tabKey) {
        ...
        case 'diamantes':
            ...
            const speciesDiamondData = diamondFursData[slug]; // Acesso corrigido aqui
        ...
        case 'super_raros':
            // Para Super Raros, a contagem é baseada nas pelagens raras que *podem* ser diamante para cada sexo
            ...
            if (sRareFurs && sDiamondFurs) {
                if (sRareFurs.macho && sDiamondFurs.macho && sDiamondFurs.macho.length > 0) {
                    possibleSuperRares += sRareFurs.macho.length; // Conta todas as pelagens raras de macho se ele pode dar diamante
                }
                if (sRareFurs.femea && sDiamondFurs.femea && sDiamondFurs.femea.length > 0) {
                    possibleSuperRares += sRareFurs.femea.length; // Conta todas as pelagens raras de fêmea se ela pode dar diamante
                }
            }
...
function updateProgressPanel(panel) {
    ...
    Object.keys(sections).forEach(key => {
        ...
        if(sectionInfo.type === 'boolean') { // Para pelagens raras
            ...
            // Calcula o total de pelagens raras que podem ser diamante para cada sexo
            Object.keys(rareFursData).forEach(slug => {
                const sRareFurs = rareFursData[slug];
                const sDiamondFurs = diamondFursData[slug];
                if (sRareFurs && sDiamondFurs) {
                    if (sRareFurs.macho && sDiamondFurs.macho && sDiamondFurs.macho.length > 0) {
                        total += sRareFurs.macho.length;
                    }
                    if (sRareFurs.femea && sDiamondFurs.femea && sDiamondFurs.femea.length > 0) {
                        total += sRareFurs.femea.length;
                    }
                }
            });
    ...
    sourceData.forEach(slug => {
        ...
        switch(categoryKey) {
            ...
            case 'super_raros':
                ...
                if (sRareFurs && sDiamondFurs) {
                    if (sRareFurs.macho && sDiamondFurs.macho && sDiamondFurs.macho.length > 0) {
                        possibleSuperRares += sRareFurs.macho.length;
                    }
                    if (sRareFurs.femea && sDiamondFurs.femea && sDiamondFurs.femea.length > 0) {
                        possibleSuperRares += sRareFurs.femea.length;
                    }
                }
...
function getCompleteTrophyInventory() {
    ...
    // Super Raros (assumindo que Super Rares are marked as completed)
...
function syncTrophyToAlbum(animalSlug, rarityType, details) {
    ...
    switch(rarityType) {
        ...
        case 'super_rares':
            ...
            // O nome da pelagem super rara é a variação (que já vem com o sufixo " Diamante" se aplicável)
...
function openGrindDetailModal(sessionId, rarityType) {
    ...
    switch (rarityType) {
        ...
        case 'super_rares':
            ...
            const speciesDiamondFurs = diamondFursData[animalSlug]; // Acesso corrigido aqui
    ...
    modal.querySelector('#grind-detail-save').onclick = () => {
        ...
        let finalVariationName = selectedValue;
        if (rarityType === 'super_rares') {
            // A super rara é a pelagem rara daquele sexo + " Diamante"
            const gender = selectedValue.toLowerCase().startsWith('macho') ? 'Macho' : 'Fêmea';
            const pureFur = selectedValue.replace(/^(Macho|Fêmea)\s/i, '').trim();
            finalVariationName = `${gender} ${pureFur} Diamante`;
        }

        const logDetails = {    
            variation: finalVariationName,
            grindCounts: session.counts    
        };
        const newLog = { id: Date.now(), variation: finalVariationName, date: new Date().toISOString() };
