...
function updateCardAppearance(card, slug, tabKey) {
    ...
    if (tabKey === 'pelagens') {
        ...
        if (!fursInfo || !counterDiv) { if(counterDiv) counterDiv.style.display = 'none'; return; }
        const totalFurs = (fursInfo.macho?.length || 0) + (fursInfo.femea?.length || 0);
        if (totalFurs === 0) { counterDiv.style.display = 'none'; return; }
        ...
        const completionRatio = totalFurs > 0 ? completedCount / totalFurs : 0;
        if (completionRatio === 0) { card.classList.add('card-unstarted'); } else if (completionRatio === 1) { card.classList.add('card-all-done'); } else { card.classList.add('card-inprogress'); }
    } else if (tabKey === 'diamantes' || tabKey === 'greats' || tabKey === 'super_raros') {
        if (animalData.completo) { card.classList.add('card-all-done'); }
    }
}
...
function toggleDetails(card, tabKey, name) {
    ...
    if (tabKey === 'pelagens') {
        ...
        detailsHTML = `<table><thead><tr><th></th><th>Pontuação do Troféu</th><th>Peso</th></tr></thead><tbody><tr><td>Machos</td><td><input type="number" name="machoScore" placeholder="0" value="${animalData.machoScore || ''}" /></td><td><input type="number" name="machoPeso" placeholder="0" value="${animalData.machoPeso || ''}" /></td></tr><tr><td>Fêmeas</td><td><input type="number" name="femeaScore" placeholder="0" value="${animalData.femeaScore || ''}" /></td><td><input type="number" name="femeaPeso" placeholder="0" value="${animalData.femeaPeso || ''}" /></td></tr></tbody></table>`;
    ...
    if (tabKey === 'diamantes') { const checkContainer = document.createElement('div'); checkContainer.className = 'check-container'; checkContainer.innerHTML = `<input type="checkbox" id="chk_${tabKey}_${slug}" name="completo" ${animalData.completo ? 'checked' : ''} /><label for="chk_${tabKey}_${slug}">Completo</label>`; detailDiv.append(checkContainer); }
    ...
    if (tabKey === 'pelagens') {
    ...
    } else if (tabKey === 'diamantes' || tabKey === 'greats') {
        detailDiv.addEventListener('click', (event) => {
            ...
            if (event.target.matches('input[name="completo"]')) { animalData.completo = event.target.checked; savedData[tabKey][slug] = animalData; updateAndSave(); }
        });
        detailDiv.addEventListener('input', (event) => {
            if(event.target.matches('input[data-fur-name]')) { const furName = event.target.dataset.furName; const fieldName = event.target.name; if (!animalData.furs) animalData.furs = {}; if (!animalData.furs[furName]) animalData.furs[furName] = {}; animalData.furs[furName][fieldName] = event.target.value; } else if (tabKey === 'diamantes') { animalData.machoScore = detailDiv.querySelector('[name="machoScore"]').value; animalData.machoPeso = detailDiv.querySelector('[name="machoPeso"]').value; animalData.femeaScore = detailDiv.querySelector('[name="femeaScore"]').value; animalData.femeaPeso = detailDiv.querySelector('[name-="femeaPeso"]').value; }
            savedData[tabKey][slug] = animalData;
            updateAndSave();
        });
    }
...
function updateProgressPanel() {
    ...
    const obtidosDiamantes = Object.values(data.diamantes || {}).filter(animal => animal.completo).length;
    ...
    document.getElementById('progress-super_raros').value = obtidosSuperRaros; document.getElementById('progress-super_raros').max = totalSuperRaros; document.getElementById('stats-super_raros').textContent = `Obtidos: ${obtidosSuperRaros} / ${totalSuperRaros}`; document.getElementById('missing-super_raros').textContent = `Faltam: ${totalSuperRaros - obtidosSuperRaros}`;
}
