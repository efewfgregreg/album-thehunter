...
function renderReserveSelectionForGrind(container, animalSlug) {
    ...
    const availableReserves = Object.entries(reservesData)
        .filter(([, reserveData]) => reserveData.animals.includes(animalSlug))
...
function openGrindDetailModal(sessionId, rarityType) {
    ...
    if (options.length === 0) {
        alert(`Nenhuma variação de '${rarityType}' encontrada para este animal.`);
        return;
    }
    ...
    modal.querySelector('#grind-detail-save').onclick = () => {
        ...
        if (!session.counts[rarityType]) {
            session.counts[rarityType] = [];
        }
