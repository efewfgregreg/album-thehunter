...
function renderSuperRareDetailView(container, name, slug) {
    ...
    // VERIFICAÇÃO SE A FÊMEA PODE SER DIAMANTE
    const canFemaleBeDiamond = diamondFursData[slug] && diamondFursData[slug].femea.length > 0;

    if (!speciesFurs || (speciesFurs.macho.length === 0 && (!canFemaleBeDiamond || speciesFurs.femea.length === 0))) {
        furGrid.innerHTML = '<p>Nenhuma pelagem de super raro listada para este animal.</p>';
        return;
    }
    ...
    if (speciesFurs.femea && canFemaleBeDiamond) {
        speciesFurs.femea.forEach(fur => {
