...
function showDetailView(name, tabKey) {
    ...
    if (tabKey === 'greats') {
        ...
        detailContent.innerHTML = `<p>Funcionalidade de detalhes para esta aba ainda não implementada neste formato.</p>`;
    }
}
...
function showAddTrophyForm(fur, slug, tabKey, onListChangeCallback) {
    ...
    document.getElementById('save-trophy-btn').onclick = () => {
        const newTrophy = {
            ...
            date: formWrapper.querySelector('[name="date"]').value || new Date().toISOString().split('T')[0]
        };
        
        if (!savedData[tabKey]) savedData[tabKey] = {};
        if (!savedData[tabKey][slug]) savedData[tabKey][slug] = { furs: {} };
